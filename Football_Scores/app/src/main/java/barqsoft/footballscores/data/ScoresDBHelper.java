...
public class ScoresDBHelper extends SQLiteOpenHelper
{
    ...
    public void onCreate(SQLiteDatabase db) {
        final String CreateScoresTable = "CREATE TABLE " + DatabaseContract.SCORES_TABLE + " ("
                ...
                + scores_table.TIME_COL + " TEXT NOT NULL," // int to text (changed)
                + scores_table.HOME_COL + " TEXT NOT NULL,"
                + scores_table.AWAY_COL + " TEXT NOT NULL,"
                + scores_table.LEAGUE_COL + " INTEGER NOT NULL,"
                + scores_table.HOME_GOALS_COL + " INTEGER NOT NULL," // text to int(changed)
                + scores_table.AWAY_GOALS_COL + " INTEGER NOT NULL,"// text to int(changed)
                + scores_table.MATCH_ID + " INTEGER NOT NULL," // league number
                + scores_table.MATCH_DAY + " INTEGER NOT NULL,"
                + scores_table.MATCH_DATE_MS + " INTEGER NOT NULL," // match datetime converted milliseconds (for future use )
